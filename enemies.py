...
class Slime(pygame.sprite.Sprite):
    def __init__(self, x, y, change_x, change_y, player_speed):
        ...
        # Load image
        self.image = pygame.image.load("slime_threat.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.topleft = (x, y)
        self.direction = "NA"
        self.chase_level = 20
    ...
    def update(self, horizontal_blocks, vertical_blocks, intersection_blocks, player_loc, all_points_info, sal_period, chase_level):
        ...
        # load slime image based on salience condition period
        if self.sal_period == "safe":
            self.image = pygame.image.load("slime_safe.png").convert_alpha()
        else:
            self.image = pygame.image.load("slime_threat.png").convert_alpha()
        ...
        if self.rect.topleft in [x[0] for x in all_points_info]:
            ...
            """
            Tracking/chasing info for enemies
            I guess this will have to pass for AI...
            """
            if pos_type in ["2way", "3way", "4way"]:
                if self.sal_period == "threat":
                    self.speed = random.choice(self.speed_options)
                else:
                    self.speed = random.choice(self.speed_options[:2]) # don't let slimes be faster than player during safe period
                ...
                # self.best_chase_dir = min(chase_options.items(), key=lambda x : x[1])[0]
                ...
                # print(self.sal_period, self.best_chase_dir, self.direction, legal_directions)

            else:
                # check that horizontal/vertical movement is proper
        ...
        # apply direction and speed changes once slime reaches intersection
